...
public class SphereScriptParser implements PsiParser, LightPsiParser {
  ...
  static boolean parse_root_(IElementType t, PsiBuilder b, int l) {
    return spherescriptFile(b, l + 1);
  }

  /* ********************************************************** */
  // property|TRIGGER|COMMENT|CRLF
  ...
  /* ********************************************************** */
  // item_*
  static boolean spherescriptFile(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "spherescriptFile")) return false;
    while (true) {
      int c = current_position_(b);
      if (!item_(b, l + 1)) break;
      if (!empty_element_parsed_guard_(b, "spherescriptFile", c)) break;
    }
    return true;
  }
